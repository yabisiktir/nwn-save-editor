...
- **Id fields read out in plain language.** Selecting a field that holds an id or a
  ...
  `Gender`, and the `GoodEvil` / `LawfulChaotic` alignment axes — shows what it refers
  to below the tree, name and (for feats and spells) description, so `Feat = 2213`
  reads as "Divine Strike", `BaseItem = 53` as "Scimitar" and `Appearance_Type = 6` as
  "Human" without opening anything. The 2DA-backed ones resolve against the
  character's own haks, so custom content reads correctly.
